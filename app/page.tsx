// app/game/[id]/page.tsx - COMPLETE VERSION WITH WINNER MODAL
'use client';
...
export default function GamePage() {
  ...
  // Check if game ended and show winner modal - ENHANCED VERSION
  useEffect(() => {
    const checkGameEnded = async () => {
      if (!game || winnerAnnouncedRef.current) return;
      
      // Check if game is finished and has a winner
      if (game.status === 'FINISHED' && game.winner) {
        console.log('🏁 Game finished! Fetching winner info...');
        
        // Mark as announced to prevent duplicate checks
        ...
        try {
          ...
          if (winnerData) {
            ...
            // Play winner announcement sound/effect
    ...
    // Cleanup function
    return () => {
      // Don't reset winnerAnnouncedRef here, we want to keep it for the session
    };
  }, [game, getWinnerInfo]);

  // Function to play winner announcement effects
  const playWinnerAnnouncement = () => {
  ...
  const handleCallNumber = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        // Show notification
        console.log(`📢 ${letter}${data.number} called!`);
  ...
  const isWinningPosition = (rowIndex: number, colIndex: number, winningCard?: WinnerInfo['winningCard']): boolean => {
    if (!winningCard?.winningPatternPositions) return false;
    const flatIndex = rowIndex * 5 + colIndex;
    return winningCard.winningPatternPositions.includes(flatIndex);
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-purple-600 to-blue-600 p-4 relative">
      {/* ENHANCED WINNER MODAL - ANNOUNCES TO ALL PLAYERS */}
      <AnimatePresence>
        {showWinnerModal && winnerInfo && (
          <motion.div
            initial={{ opacity: 0 }}
      ...
      {isWinnerLoading && (
        <div className="fixed inset-0 bg-black/90 flex items-center justify-center z-40">
          <div className="bg-gradient-to-br from-purple-700 to-blue-800 rounded-2xl p-8 text-center">
            <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-white mx-auto mb-4"></div>
            <p className="text-white font-medium text-lg">🎉 Announcing Winner...</p>
            <p className="text-white/60 text-sm mt-2">Preparing victory celebration!</p>
          </div>
        </div>
      )}
